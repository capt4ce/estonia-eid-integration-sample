...
export class EstoniaEid {
  ...
  // extract information from the eID
  async extractInfo(cert: any) {
    ...
    const [lastName, firstName, personalCode] = certificate
      .getParam()
      .subject.array[1][0].value.split(',');
  ...
  async signData(data: string) {
    ...
    try {
      ...
      try {
        const signatureResponse: any = await this.hwcrypto.sign(
          cert,
